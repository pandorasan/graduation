...
def inference(image):
    ...
    return hidden[NUM_LAYERS], hidden[1], w0, hidden[2]
...
for step in range(MAX_STEP):
    ...
    if step%100 == 0:
        ...
        loss = gene_loss
        train_accuracy = accuracy.eval(feed_dict={x_data: batch[0], y_data: batch[1]})
        accu = train_accuracy
...
plt.ioff()
saver.save(sess, 'deep_thin/model.ckpt')
