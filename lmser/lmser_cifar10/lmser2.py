...
hehe = lmser2_inference.inference(x_data, IMAGE_PIXELS,
  FLAGS.hidden1, FLAGS.hidden2, y_data, NUM_CLASSES, reg_constant=FLAGS.reg_constant)
logits = hehe[0]
x_pre = tf.nn.relu(tf.matmul(hehe[1], tf.transpose(hehe[2])))


loss = lmser2_inference.loss(logits, y_data)
# Operation for the training step
train_step = lmser2_inference.training(loss, FLAGS.learning_rate)

# Operation calculating the accuracy of our predictions
accuracy = lmser2_inference.evaluation(logits, y_data)
